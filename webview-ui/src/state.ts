...
export type ProjectGateState = { advisory: boolean; partial: boolean; summary: string; files: ProjectGateFileView[]; projectErrors: string[]; dod: string[] };
...
function applyExt(state: UIState, msg: ExtToWebview): UIState {
  switch (msg.type) {
    ...
    case "project/gate":
      ...
      return state.project ? { ...state, project: { ...state.project, gate: { advisory: msg.advisory, partial: msg.partial, summary: msg.summary, files: msg.files, projectErrors: msg.projectErrors, dod: msg.dod } } } : state;
    case "project/done":
