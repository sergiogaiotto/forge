...
// sem `./` inicial. NÃO mexe em caixa (paths são case-sensitive em Linux).
...
// ancestral de um `.py` é um pacote; a RAIZ (string vazia) nunca vira pacote; diretórios que já têm um
// `__init__.py` entre as próprias propostas são pulados (não sobrescreve o do modelo). Puro.
export function syntheticInitDirs(relPaths: string[]): string[] {
  ...
  return [...needed].filter((d) => !alreadyPkg.has(d)).sort();
}
...
export function summarizeGate(checks: GateCheckResult[]): ProjectGateSummary {
  ...
  const blockedCount = fileErrors.length + (projectErrors.length ? 1 : 0);
  ...
  const summary = advisory
    ...
    : blockedCount > 0
      ? `Gate reprovou: ${fileErrors.length} arquivo(s) não compilam/importam${projectErrors.length ? " (+ falha ampla)" : ""}. O "Aplicar" deles está bloqueado até corrigir.`
      : mypyRan
